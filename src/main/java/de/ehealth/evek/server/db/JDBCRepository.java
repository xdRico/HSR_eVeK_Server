package de.ehealth.evek.server.db;

import static de.ehealth.evek.server.db.SQL.DELETE_FROM;
import static de.ehealth.evek.server.db.SQL.INSERT_INTO;
import static de.ehealth.evek.server.db.SQL.SELECT;
import static de.ehealth.evek.server.db.SQL.UPDATE;
...
import de.ehealth.evek.api.entity.User.LoginUser;
...
import de.ehealth.evek.server.db.SQL.InsertBuilder;
import de.ehealth.evek.server.db.SQL.UpdateBuilder;
...
//				stmt.execute(CREATE_TABLE_PROTOCOL);
		...
		private static String selectAllCaseInsensitiveSQL(String table, String key, String keyData) {
			return String.format("SELECT * FROM \"%s\" WHERE LOWER(\"%s\") = '%s'", table, key, keyData.toLowerCase());
		}
...
//		}
		...
		private static String insertSQL(User user) {
			 return INSERT_INTO("user")
					 .VALUE("userId", user.id().value().toString().toLowerCase())
...
//		}
		...
		public List<ServiceProvider> getServiceProvider(ServiceProvider.Filter filter) {
			...
			filter.isHealthcareProvider().ifPresent(ref -> query.WHERE("isHealthcareProvider", ref));
			filter.isTransportProvider().ifPresent(ref -> query.WHERE("isTransportProvider", ref));
			filter.name().ifPresent(ref -> query.WHERE("name", ref));
...
//		}
		...
		public COptional<User> getUser(Id<User> id) {
			try (var stmt = conn.createStatement();
					var rs = stmt.executeQuery(selectAllSQL("user", "userId", id.value().toLowerCase()))) {
				if(!rs.isClosed())
					if (rs.next()) 
		...
		public COptional<User> getUser(String id) {
			try (var stmt = conn.createStatement();
					var rs = stmt.executeQuery(selectAllCaseInsensitiveSQL("user", "userId", id.toLowerCase()))) {
				if(!rs.isClosed())
					if (rs.next()) 
						return COptional.of(readUser(rs));
			} catch (SQLException e) {
